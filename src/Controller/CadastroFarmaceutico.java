...
public class CadastroFarmaceutico implements Initializable {
    ...
    void cadastrar(ActionEvent event) {
        if (cpf.getText().isEmpty() || matricula.getText().isEmpty() || endereco.getText().isEmpty() || telefone.getText().isEmpty() || senha.getText().isEmpty() || salario.getText().isEmpty() || sessao.getItems().isEmpty()){
            alerta.Warning("Atenção", "Preencha todos os campos");
        }
        else{
            try {
                if (dao.salvar(new Farmaceutico(cpf.getText(), matricula.getText(), senha.getText(), Float.valueOf(salario.getText()), sessao.getValue(), telefone.getText(), endereco.getText(), crf.getText()))){
                    alerta.Confirmation("Parabéns", "Cadastro realizado com sucesso");
                }else alerta.Error("Erro", "Cadastro não relizado");
            } catch (SQLException e) {
                alerta.Error("Erro",  "Funcionario ja existe");
